...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", print_result=True):
    ...
    # EDITION: Remove DATE LABEL during evaluation for transfer learning
    new_out_label_list = []
    for out_label in out_label_list:
        tmp = []
        for out in out_label:
            if "DATE" in out:
                tmp.append("O")
            else:
                tmp.append(out)
        new_out_label_list.append(tmp)
    out_label_list = new_out_label_list
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        # Save results
        output_dev_results_file = os.path.join(args.output_dir, args.dev_result_file)
        with open(output_dev_results_file, "w") as writer:
            for key in sorted(result.keys()):
                writer.write("{} = {}\n".format(key, str(result[key])))
        # Save predictions
        output_dev_predictions_file = os.path.join(args.output_dir, args.dev_prediction_file)
        with open(output_dev_predictions_file, "w", encoding='utf-8') as writer:
            with open(os.path.join(args.data_dir, "dev.txt"), "r", encoding='utf-8') as f:
                example_id = 0
                for line in f:
